...
export default function ClassCard({ cls, isAdmin, onDelete }: ClassCardProps) {
  ...
  return (
    <>
      <div className={`rounded-2xl border p-5 shadow-sm transition-shadow hover:shadow-md ${
        ...
        {low && (
          <div className="mb-3 flex items-center gap-1.5 rounded-full bg-amber-100 px-3 py-1 text-xs font-medium text-amber-700 w-fit">
            <span>⚠</span> Low Attendance
          </div>
        )}
        ...
        <div className="mt-4 flex gap-2">
          ...
          {isPast && (
            <span className="flex-1 rounded-xl bg-gray-50 py-2.5 text-center text-sm text-gray-400">
              Class Ended
            </span>
          )}
          {isAdmin && (
